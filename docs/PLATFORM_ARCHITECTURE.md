...
FIT LOG は旧GAS版から独立した Cloudflare D1 + Pages API へ移行済みです。
MAYAのD1へ複製・同期せず、相談があったときだけ共有APIを直接読みます（2026-09-24）。

MAYAから使う道具は次の4つです（2026-09-26）。

| 道具 | 読むもの |
| --- | --- |
| `fitlog_day` | 指定日の体重・体脂肪率、測定日と出所、食事PFC、ジム、有酸素、屋外運動 |
| `fitlog_progress` | 14/28/42日の体組成、30/90/180日の実測TDEE、週別運動量、筋力変化、体脂肪目標 |
| `fitlog_weekly` | 直前の完了週のジム日数、筋トレ量、有酸素、屋外運動、食事記録、体重、自己ベスト |
| `fitlog_exercise` | 指定種目の前回、自己ベスト、推定1RM、直近28日のボリューム |

日次の体重・体脂肪率は、対象日に記録が無いと直近値が表示されることがあります。
MAYAには値だけでなく実際の測定日、手入力かAppleヘルスケア由来か、最終同期時刻も渡し、
直近値を「今日の測定」と言わせません。週次取得は `factsOnly=1` を付け、FIT LOG側の
AIコメント生成・保存と既読変更を起こさない読み取り専用経路にします。
