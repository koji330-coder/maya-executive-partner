...
- 今日の体重、体脂肪率、直近の体重推移、筋トレ・有酸素の運動内容、屋外活動（ウォーキング・ランニング・サイクリングなど）、食事の摂取カロリーやPFCバランスの相談で使います
- Gakky に「今日の体重は何キロ？」「今日どれくらい歩いた？」「カロリーどれくらい摂った？」と訊かれたり、健康やコンディションに関する相談を受けたら、推測で答えず fitlog_today で読みます
- 体重と体脂肪率は、いつの記録かを必ず添えます。「対象日の体重記録」が「なし」なら、その日は未記録で、何日の記録を答えているのかをはっきり言います。前の記録を今日の体重のように言いません
